...
8. **🔴 Relatório HTML obrigatório antes do PR**: toda branch que altera código OU docs DEVE gerar relatório em `docs/reports/<data>/<prefixo>-YYYY-MM-DD-<nome>.html` antes do PR. Prefixos: `PR<num>`, `Sprint<letra>`, `auto`, `fix`, `feat`, `docs`, `chore`. Template: `/report`. O relatório DEVE incluir seção **🎯 Benefícios** com o impacto da mudança (ex: menos alertas, UX mais limpo, economia de tokens). Use `--benefits` no script. Execute `node scripts/verify-docs.mjs` para verificar integridade dos docs antes do PR.

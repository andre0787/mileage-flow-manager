...
8. **🔴 Relatório HTML obrigatório antes do PR**: toda branch que altera código OU docs DEVE gerar relatório em `docs/reports/<data>/<prefixo>-YYYY-MM-DD-<nome>.html` antes do PR. Prefixos: `PR<num>`, `Sprint<letra>`, `auto`, `fix`, `feat`, `docs`, `chore`. Template: `/report`. O relatório DEVE incluir seções: **🏷️ Nível de Risco**, **✅ Checklist de Revisão**, **🎯 Benefícios**, **🏢 Impacto no Negócio**, **⚡ Consumo de Tokens**, **📋 Detalhamento por Item**. Use `--benefits`, `--impact` e `--rows` no script. O relatório é **gerado automaticamente** pelo `npm run pre-pr` (primeiro passo da validação). A nomenclatura DEVE conter o número do PR: `PR<num>-YYYY-MM-DD-<nome>.html`. Execute `node scripts/verify-docs.mjs` para verificar integridade dos docs antes do PR.

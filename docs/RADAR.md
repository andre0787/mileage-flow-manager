# 🔭 Radar de Vulnerabilidades

> Atualizado em: 2026-08-01T14:51:00.639Z

## ⚠️  Vulnerabilidades Ativas (1 pacotes)

### 🟠 react-router@7.18.2 (high)

| # | Advisory | Range | Severidade |
|---|----------|-------|------------|
| 1 | React Router: RSC Mode CSRF Bypass Allows Action Execution Before 4... [🔗](https://github.com/advisories/GHSA-qwww-vcr4-c8h2) | `>=7.12.0 <8.3.0` | high |

---

**Para resolver:**
- `npm update <pacote>` — se houver versão segura
- `npm audit fix` — corrige automaticamente (pode quebrar)
- Verificar advisories individuais nos links acima

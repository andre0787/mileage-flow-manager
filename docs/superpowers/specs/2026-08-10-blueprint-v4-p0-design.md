...
| (legado) src/store/slices/milesSlice.ts | **Não existe.** Entradas/milhas em (legado) src/hooks/useDatabase/entries.ts + `src/lib/metrics.ts` | P1 migra de `hooks/useDatabase` → `src/features/entradas/` |

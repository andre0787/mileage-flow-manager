...
const CHARTS_PACKAGES = ["node_modules/recharts", "node_modules/d3-", "node_modules/victory"];
const UI_PACKAGES = ["node_modules/@radix-ui", "node_modules/lucide-react"];
...
export default defineConfig(({ mode }) => ({
  ...
  build: {
    rollupOptions: {
      ...
      output: {
        manualChunks: (id) => {
          if (VENDOR_PACKAGES.some((pkg) => id.includes(pkg))) return "vendor";
          if (CHARTS_PACKAGES.some((pkg) => id.includes(pkg))) return "charts";
          if (UI_PACKAGES.some((pkg) => id.includes(pkg))) return "ui";
          return undefined;
        },
      },
    },

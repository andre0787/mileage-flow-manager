...
// Escreve diagnósticos em stderr (fail-open) sem console.* no diff (rule-30).
function warn(...args: unknown[]) {
  process.stderr.write(`[rtk] ${args.join(" ")}\n`)
}
...
export default async function (pi: ExtensionAPI) {
  ...
  if (ver.code !== 0) {
    warn("rtk binary not found in PATH — extension disabled")
    return
  }
  ...
  if (parsed) {
    ...
    if (major === 0 && minor < MIN_SUPPORTED_RTK_MINOR) {
      warn(`rtk ${ver.stdout.trim()} is too old (need >= 0.23.0) — extension disabled`)
      return
    }
  }
  ...
  pi.on("tool_call", async (event, ctx) => {
    try {
      ...
      warn("unexpected error in tool_call handler; passing through command", err)
      return
    }
  })
}

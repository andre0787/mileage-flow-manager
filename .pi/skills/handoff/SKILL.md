...
1. Analisa o estado atual da conversa
2. Atualiza `HANDOFF.md` com:
   - Goal atual
   - Progress (Done, In Progress, Blocked)
   - Key Decisions tomadas
   - Next Steps
   - Critical Context (branch, commits, arquivos modificados, PRs abertos)
3. Sugere `/new` para começar sessão limpa
...
1. Executar `git branch --show-current` para saber a branch atual
2. Executar `git log --oneline -1` para último commit
3. Executar `git status --short` para arquivos modificados

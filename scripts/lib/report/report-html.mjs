/**
 * report-html.mjs — Geração do briefing executivo HTML.
 *
 * generateHTML() monta o deck fullscreen com 5 slides + CSS + JS de navegação.
 * Template string gigante extraído do monolitico generate-report.mjs.
 *
 * deps: report-cli (PREFIX, REPORT_DATE), report-git (escapeHTML, TYPE_BENEFIT,
 *        TYPE_IMPACT), report-metrics (sessionHealth)
 */

import { PREFIX, REPORT_DATE } from "./report-cli.mjs";
import { TYPE_BENEFIT, TYPE_IMPACT } from "./report-git.mjs";
import { sessionHealth } from "./report-metrics.mjs";
...
export function generateHTML({
  ...
  // ── Risk level (auto-detectado dos arquivos) ──────────────────────
  ...
  const hasOnlyDocs = filePaths.every(
    (p) =>
      p.startsWith("docs/") || p.startsWith("scripts/") || p.startsWith(".pi/") || p.includes(".md"),
  );
      ...
      : [
          {
            ...
            fix: task,
            benefit: impactProduto || "Mudança validada pelo fluxo de qualidade",
            impact: impactNegocio || "Risco controlado antes da produção",
  ...
  // ── CSS + HTML + JS (template string extraído do monolítico) ─────
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">

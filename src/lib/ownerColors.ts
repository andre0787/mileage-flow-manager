 ...
 *
 * Cada dono recebe uma cor determinística derivada do NOME (hash FNV-1a) —
 * o mesmo dono tem a mesma cor em Contas, Entradas e Vendas, sem precisar de
 * campo no banco. A cor é um reforço visual: o nome do dono continua sempre
 * renderizado ao lado.
 *
 * Uso:
 *   ownerColor("Fulano")        → "#5B72C4" (hex sólido)
 *   ownerColorSoft("Fulano")    → "rgba(91,114,196,0.13)" (fundo suave)
 *   ownerColorBorder("Fulano")  → "rgba(91,114,196,0.5)" (borda)
 *
...
/** Cor sólida do dono (hex). Nomes vazios caem no índice 0. */
...
export function ownerColorSoft(name: string): string {
  return withAlpha(ownerColor(name), 0.13);
}

/** Borda do dono (alpha ~50%) para destaques de card/linha. */
export function ownerColorBorder(name: string): string {
  return withAlpha(ownerColor(name), 0.5);
}

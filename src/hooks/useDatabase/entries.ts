    ...
    mutationFn: async (entry: PointEntry) => {
      ...
      const { error } = await supabase.from("entries").insert({
        ...
        description: serializeDescription({
          cartAmount: entry.cartAmount, cartCost: entry.cartCost, entryStatus: entry.entryStatus,
          parentEntryId: entry.parentEntryId, recurrenceInterval: entry.recurrenceInterval,
      ...
      if (!isAguardando && entry.recurrenceInterval && entry.recurrenceEnd) {
        ...
        for (const fe of futureEntries) {
          await supabase.from("entries").insert({
            ...
            description: serializeDescription({
              cartAmount: fe.cartAmount, cartCost: fe.cartCost, entryStatus: fe.entryStatus,
              parentEntryId: fe.parentEntryId, recurrenceInterval: fe.recurrenceInterval,
  ...
  return useMutation({
    mutationFn: async (entry: PointEntry) => {
      ...
      const newDesc = serializeDescription({
        cartAmount: currentDesc.cartAmount, cartCost: currentDesc.cartCost,
        parentEntryId: currentDesc.parentEntryId, recurrenceInterval: currentDesc.recurrenceInterval,
      ...
      const { data: dest, error: accErr } = await supabase
        .from("accounts").select("balance, total_invested").eq("id", entry.accountId).maybeSingle();
    ...
    onError: (err) => { console.error("[confirmEntry]", err); },
  });
}
  ...
  return useMutation({
    mutationFn: async ({ oldEntry, updates }: { oldEntry: PointEntry; updates: Partial<PointEntry> }) => {
      ...
      const { error: insErr } = await supabase.from("entries").insert({
        ...
        description: serializeDescription({
          cartAmount: merged.cartAmount, cartCost: merged.cartCost, entryStatus: merged.entryStatus,
          parentEntryId: merged.parentEntryId, recurrenceInterval: merged.recurrenceInterval,

...
export default function ProgramSection({
  ...
  const handleSave = () => {
    ...
    if (form.passengerCycleType !== "none" && !form.maxPassengers) {
      setError("Máx. Passageiros é obrigatório para controle ativo");
      return;
    }
    if (form.passengerCycleType === "dias" && !form.passengerCycleDays) {
      setError("Janela em dias é obrigatória para ciclo por dias");
      return;
    }
    const data = {
      name: form.name,
      type: form.type,
      maxPassengers:
        form.passengerCycleType !== "none" && form.maxPassengers
          ? parseInt(form.maxPassengers)
          : undefined,
      passengerCycleType: form.passengerCycleType !== "none" ? form.passengerCycleType : undefined,
      passengerCycleDays:
        form.passengerCycleType === "dias" && form.passengerCycleDays
          ? parseInt(form.passengerCycleDays)
          : undefined,
    };
    if (editing) {
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={form.type}
                  onValueChange={(value) => setForm({ ...form, type: value as Program["type"] })}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={form.passengerCycleType}
                  onValueChange={(value) =>
                    setForm({ ...form, passengerCycleType: value as "none" | "anual" | "dias" })
                  }
                >
                  <SelectTrigger>
              ...
              {form.passengerCycleType === "dias" && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min="1"
                    value={form.passengerCycleDays}
                    onChange={(e) => setForm({ ...form, passengerCycleDays: e.target.value })}
                    placeholder="Ex: 365"
                  />
                </div>
              )}
                        ...
                        <DeleteConfirmDialog
                          trigger={
                            <Button
                              ...
                              disabled={accounts.some((a) => a.programId === program.id)}
                            >
                          ...
                          description={
                            accounts.some((a) => a.programId === program.id)
                              ? `Não é possível excluir o programa "${program.name}" pois existem contas vinculadas. Remova as contas primeiro.`
                              : `Tem certeza que deseja excluir o programa "${program.name}"? Esta ação não pode ser desfeita.`
                          }
